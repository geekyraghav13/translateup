...
const Card = ({ scrollYProgress, index, feature }: { scrollYProgress: MotionValue<number>, index: number, feature: Feature }) => {
    const start = 0.6 + index * 0.03;
    const end = 0.75 + index * 0.03;
...
export const HeroAnimation = () => {
  ...
  const mainAnimationOpacity = useTransform(scrollYProgress, [0, 0.1], [0, 1]);
  const phoneX = useTransform(scrollYProgress, [0.1, 0.3], ['-30vw', '0vw']);
  const textOpacity = useTransform(scrollYProgress, [0.1, 0.3], [1, 0]);
  const rotate = useTransform(scrollYProgress, [0.3, 0.5], [90, 0]);
  
  const springConfig = { damping: 30, stiffness: 200, mass: 0.5 };
  const smoothX = useSpring(phoneX, springConfig);
  const smoothRotate = useSpring(rotate, springConfig);
  const scale = useTransform(scrollYProgress, [0, 0.8, 1], [0.8, 1, 0.9]);
